...
//#define EXTRABAUD 1000000

unsigned long extrabaud;
int extrabaud_line = 2;

static int __init extrabaud_setup(char *str)
{
	extrabaud = simple_strtoul(str, NULL, 10);

	return 1;
}

static int __init extrabaud_line_setup(char *str)
{
	extrabaud_line = simple_strtoul(str, NULL, 10);
	return 1;
}

__setup("extrabaud=", extrabaud_setup);
__setup("extrabaud_line=", extrabaud_line_setup);
...
uart_get_divisor(struct uart_port *port, unsigned int baud)
{
	...
	if (extrabaud_line == port->line) {
	
		if( baud == 115200 && extrabaud ) {
			baud = extrabaud;	
		}
