...
extern unsigned int extrabaud;
extern int extrabaud_line;
...
int console_detect(char *str)
{
	...
	while ((next = strchr(next, 'c')) != NULL) {
		/* check for begin of a new argument */
		if ((saved_command_line == next || isspace(*(next-1))) 
				&& !strncmp(next, CONSOLE_NAME, i)) {
			start = next;
			break;
		} else {
			next++;
