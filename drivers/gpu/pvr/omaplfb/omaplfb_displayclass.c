...
// MAX_BUFFERS_FLIPPING Can not be defined statically, it depends on the available framebuffer size and on the requested size
static int MAX_BUFFERS_FLIPPING=3;
...
void updateOverlayPosition(int x, int y)
{
    ...
    for(fb_idx = 0; fb_idx < FRAMEBUFFER_COUNT; fb_idx++)
    {   
        psDevInfo = &pDisplayDevices[fb_idx];
        ...
        psPVRFBInfo->sSysAddr.uiAddr = psLINFBInfo->fix.smem_start + psLINFBInfo->fix.line_length * y + psLINFBInfo->var.bits_per_pixel/8 * x; 
        psDevInfo->sFBInfo.sSysAddr.uiAddr = psPVRFBInfo->sSysAddr.uiAddr;

        /* execute the change on framebuffer */
        {
            struct fb_info * framebuffer = psDevInfo->psLINFBInfo;
            struct omapfb_info *ofbi = FB2OFB(framebuffer);
            struct omapfb2_device *fbdev = ofbi->fbdev;
            unsigned long fb_offset =
                (unsigned long)psDevInfo->sFBInfo.sSysAddr.uiAddr - psDevInfo->sSystemBuffer.sSysAddr.uiAddr;
...
static OMAP_ERROR InitDev(OMAPLFB_DEVINFO *psDevInfo, int fb_idx)
{
	...
	MAX_BUFFERS_FLIPPING = psLINFBInfo->fix.smem_len / (psLINFBInfo->var.yres*psLINFBInfo->var.xres*psLINFBInfo->var.bits_per_pixel/8);
    printk("MAX_BUFFERS_FLIPPING = %d!!!!!!!!!!!!!!!\n", MAX_BUFFERS_FLIPPING);
...
void updateOverlay(int x, int y, int w, int h)
{
    int fb_idx;
    struct fb_info *psLINFBInfo;
    OMAPLFB_DEVINFO *psDevInfo;
	...
	if (psLINFBInfo->var.width < 0 || psLINFBInfo->var.height < 0) {
        ...
        if (psDevInfo->sDisplayInfo.ui32MaxSwapChainBuffers > 1) {
            psPVRFBInfo->sSysAddr.uiAddr += psLINFBInfo->fix.line_length * y + psLINFBInfo->var.bits_per_pixel/8 * x; 
        }
        psDevInfo->sFBInfo.sSysAddr.uiAddr = psPVRFBInfo->sSysAddr.uiAddr;
        ...
        psDevInfo->sSystemBuffer.ulBufferSize =
            psDevInfo->sFBInfo.ulRoundedBufferSize;
        DEBUG_PRINTK("Buffers available: %lu (%lu bytes per buffer)",
            psDevInfo->sDisplayInfo.ui32MaxSwapChainBuffers,
            psDevInfo->sFBInfo.ulBufferSize);
