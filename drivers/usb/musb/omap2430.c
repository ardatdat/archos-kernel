...
static void musb_do_idle(unsigned long _musb)
{
	...
	case OTG_STATE_A_WAIT_BCON:
		...
		/* we don't drop session here - otherwise if you disconnect a device from the host port
		 * the USB host stops working. We don't want to wait for USB devices implementing SRP...
		 */
...
static void omap_vbus_power(struct musb *musb, int is_on, int sleeping)
{
	...
	if ( plat->set_vbus ) {
		(plat->set_vbus)(musb->controller, is_on);
	}
}
...
/*
 * functions to access the PHY through the musb ULPI interface
 *
 * 	mixture of what we had in Gen7 and what is proposed upstream
 * 	in http://kerneltrap.org/mailarchive/linux-usb/2010/5/20/6261360
 */

static int omap_phy_read(struct musb *musb, int addr, u8 *reg)
{
	...
	*reg = 0;
	...
	musb_writeb(musb->mregs, MUSB_ULPI_REG_ADDR, addr);
	musb_writeb(musb->mregs, MUSB_ULPI_REG_CONTROL, MUSB_ULPI_REG_REQ | MUSB_ULPI_RDN_WR);

	timeout = jiffies + 1*HZ;
	while(time_before(jiffies, timeout)) {
		u8 tmp = musb_readb(musb->mregs, MUSB_ULPI_REG_CONTROL);
		if (tmp & MUSB_ULPI_REG_CMPLT)
			break;
		udelay(2);
	}
	...
	r = musb_readb(musb->mregs, MUSB_ULPI_REG_CONTROL);
	r &= ~MUSB_ULPI_REG_CMPLT;
	musb_writeb(musb->mregs, MUSB_ULPI_REG_CONTROL, r);

	*reg = musb_readb(musb->mregs, MUSB_ULPI_REG_DATA);

	return 0;
}

static int omap_phy_write(struct musb *musb, int addr, u8 value)
{
 	unsigned long timeout;
	u8	r;
	u8	power;
	...
	musb_writeb(musb->mregs, MUSB_ULPI_REG_ADDR, addr);
	musb_writeb(musb->mregs, MUSB_ULPI_REG_DATA, value);
	musb_writeb(musb->mregs, MUSB_ULPI_REG_CONTROL, MUSB_ULPI_REG_REQ);
...
int __init musb_platform_init(struct musb *musb)
{
	...
	/* reset phy - this is a work-around for errata ID: i445 
	   "HS USB OTG software reset is not fully functionnal" */
	omap_phy_write( musb, 0x5, (1 << 6)|(1 << 5));
	l = omap_readl(OTG_SYSCONFIG);
	if (cpu_is_omap3630()) {
		/*
...
int musb_platform_exit(struct musb *musb)
{
	...
	/* it looks like we need to reset musb in case we are leaving host mode at least */
	l = omap_readl(OTG_SYSCONFIG);
	l |= SOFTRST;
	omap_writel(l, OTG_SYSCONFIG);
	while (!(RESETDONE & omap_readl(OTG_SYSSTATUS)))
