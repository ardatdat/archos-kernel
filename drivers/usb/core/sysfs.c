...
#ifdef CONFIG_USB_SUSPEND
static ssize_t
show_suspend_support(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct usb_device *udev = to_usb_device(dev);

	return sprintf(buf, "%d\n", udev->suspend_support);
}

static ssize_t
set_suspend_support(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct usb_device *udev = to_usb_device(dev);
	int value;

	if (sscanf(buf, "%d", &value) != 1 || (value != 1 && value != 0))
		return -EINVAL;

	udev->suspend_support = value;

	return count;
}

static DEVICE_ATTR(suspend_support, S_IRUGO | S_IWUSR, show_suspend_support, set_suspend_support);
