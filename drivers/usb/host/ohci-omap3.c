...
static void ohci_omap_stop(struct usb_hcd *hcd)
{
	...
	/* 
	 * need to call ohci_stop(), otherwise debugfs is not pruned
	 * and will cause kernel panic when accessed, prevent module
	 * to be loaded again.
	 */
	ohci_stop(hcd);
}
...
static int ohci_omap_clock_power(struct ohci_omap_clock_defs *ohci_clocks, int on)
{
	u32 usbtll_sysconfig;
	u32 uhh_sysconfig;
...
static irqreturn_t ohci_omap_irq(struct usb_hcd *hcd)
{
	...
	if (unlikely(ohci_clocks->suspended)) {
		...
		ohci_omap_clock_power(ohci_clocks, 1);
	}
	return ohci_irq(hcd);
}
...
static int omap_ohci_bus_suspend(struct usb_hcd *hcd)
{
	...
	if (!ohci_clocks->suspended) {
		...
		ohci_omap_clock_power(ohci_clocks, 0);
