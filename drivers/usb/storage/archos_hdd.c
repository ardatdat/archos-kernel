/* Special functions for Archos builtin hard disk
 *
 *   (c) 2008-2010 Archos S.A.
 *
...
static int archos_dpm_suspend(struct hdd_dpm_ops *dpm_ops)
{
	struct archos_hdd *hdd = to_archos_hdd(dpm_ops);
	struct us_data *us = hdd->us;
	int rc = 0;
#ifdef SUSPEND_EHCI
	pm_message_t msg = { .event=PM_EVENT_SUSPEND };
#endif
	
	printk(KERN_DEBUG "archos_dpm_suspend\n");
	...
	msleep(100);
#ifdef SUSPEND_EHCI
	hcd_bus_suspend(hdd->pusb_parent, msg);
	msleep(10);
...
static int archos_dpm_resume(struct hdd_dpm_ops *dpm_ops)
{
	struct archos_hdd *hdd = to_archos_hdd(dpm_ops);
	struct us_data *us = hdd->us;	
	int wait_time;
#ifdef SUSPEND_EHCI
	pm_message_t msg = { .event=PM_EVENT_RESUME };
#endif

	printk("archos_dpm_resume\n");
	...
	hcd_bus_resume(hdd->pusb_parent, msg);
	msleep(100);
#endif
	/* power to the drive ... */
	usbsata_power(1);
...
static int archos_dpm_sync(struct hdd_dpm_ops *dpm_ops)
{
	struct archos_hdd *hdd = to_archos_hdd(dpm_ops);
	struct us_data *us = hdd->us;
	...
	for (i = gd->first_minor; i < gd->first_minor + gd->minors; i++) {
		...
		//printk("archos_dpm_sync: syncing %i:%i\n", gd->major, i);
		if (bdev)
			sync_blockdev(bdev);
		else
			printk("archos_dpm_sync: minor %i not found\n", i);
	}

	return 0;
}

static unsigned long archos_dpm_iocount(struct hdd_dpm_ops *dpm_ops)
{
	struct archos_hdd *hdd = to_archos_hdd(dpm_ops);
 ...
 */
static void archos_hdd_pm_hook(struct us_data *us, int state)
{
...
static int archos_sata_reset( struct us_data *us )
{
	int wait_time;
...
static void archos_hdd_exit(void* __hdd)
{
...
int archos_hdd_init(struct us_data *us)
{
	...
	if (strncmp(us->pusb_dev->bus->bus_name, USB_HOST_NAME, strlen(USB_HOST_NAME)) != 0) {
		/* clear the non_removable flag, if it is not our device */
		us->fflags &= ~US_FL_NOT_REMOVABLE;
		return 0;
	}
	...
	hdd->us = us;
	hdd->dpm_ops.suspend  = archos_dpm_suspend;
	hdd->dpm_ops.resume   = archos_dpm_resume;
	hdd->dpm_ops.sync     = archos_dpm_sync;
	hdd->dpm_ops.get_iocount = archos_dpm_iocount;
	...
	us->extra_destructor  = archos_hdd_exit;
	us->extra             = hdd;
	us->suspend_resume_hook = archos_hdd_pm_hook;
	us->device_reattach   = archos_sata_reset;
