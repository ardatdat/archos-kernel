...
#define KEY_UNLOCK1			0xFC
#define KEY_UNLOCK2			0x96
...
add_children(struct twl4030_platform_data *pdata, unsigned long features)
{
	...
	if (twl_has_usb() && pdata->usb) {
		child = add_child(0, "twl4030_usb",
				...
				/* irq0 = USB_PRES, irq1 = ACCESSORY */
				/* note that twl-4030-usb does not use the USB irq */
				pdata->irq_base + 8 + 2, pdata->irq_base + 2);
#else
	...
	if (twl_has_pwrbutton()) {
		child = add_child(1, "twl4030_pwrbutton",
				pdata->pwrbutton, 0, true, pdata->irq_base + 8 + 0, 0);
		if (IS_ERR(child))
			return PTR_ERR(child);
	}
...
twl4030_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* unlock the idreg registers and print out the chip revision */
	if (1) {
		u8 regs[4] = { REG_IDCODE_0_7, REG_IDCODE_8_15,
				REG_IDCODE_16_23, REG_IDCODE_24_31 };
		u8 val[4];
		int i;
		
		twl4030_i2c_write_u8(TWL4030_MODULE_INTBR, 
				0x49, REG_UNLOCK_TEST);
		
		for (i=0; i < 4; i++)
			twl4030_i2c_read_u8(TWL4030_MODULE_INTBR, &val[i],
					regs[i]);
		
		printk("twl4030 idcode %02x%02x%02x%02x\n",
...
static const struct i2c_device_id twl4030_ids[] = {
	...
	{ "tps65921", TPS_SUBSET },	/* fewer LDOs; no codec or charger */
	{ /* end of list */ },
};
