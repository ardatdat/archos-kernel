...
/*
 * The resizer may need to read more pixels per line than the real length of a line,
 * but it will never be more than 3 pixels (6 bytes). It can lead to iommu errors for
 * the last line only (and if the processed input buffer is exactly 1280*720 - 16 bits).
 * So we need to add 6 bytes to please the resizer.
 */
#define OMAP_VOUT_MAX_BUF_SIZE (VID_MAX_WIDTH*VID_720P_HEIGHT*2 + 6)
...
static u32 video2_numbuffers = 0;
static u32 video1_bufsize = OMAP_VOUT_MAX_BUF_SIZE;
static u32 video2_bufsize = OMAP_VOUT_MAX_BUF_SIZE;
static u32 video1_maxbuffers = OMAP_VOUT_MAX_BUFFERS;
static u32 video2_maxbuffers = 0;
...
module_param(video1_maxbuffers, uint, S_IRUGO);
MODULE_PARM_DESC(video1_maxbuffers, "Max number of buffers for Video1 device.");

module_param(video2_maxbuffers, uint, S_IRUGO);
MODULE_PARM_DESC(video1_maxbuffers, "Max number of buffers for Video2 device.");
...
{
	int j;
	u32 video_maxbuffers;

	video_maxbuffers = (vout->vid == OMAP_VIDEO1) ?
		video1_maxbuffers : video2_maxbuffers;
	for (j = 0; j < video_maxbuffers; j++) {
		...
		int outh, u32 addr)
{
	...
	if (vout->flg_720 != VIDEO_720_DISABLE ||
	    vout->use_isp_rsz_for_downscale) {
			  ...
			  unsigned int *size)
{
	...
	if ( /*(rotation_enabled(vout->rotation)) &&*/ *count > video_maxbuffers) {
		*count = video_maxbuffers;
	}
			    ...
			    enum v4l2_field field)
{
	...
	if (vout->flg_720 == VIDEO_720_ENABLE ||
		...
		/*Start resizing*/
		if (!rotation_enabled(vout->rotation)) {
			int out_w = (vout->win.w.width + 0x0f) & ~0x0f;
			ret = rsz_begin(vb->i, vb->i,
					out_w * 2,
					(u32)vout->smsshado_phy_addr[vb->i],
					vout->buf_phy_addr[vb->i],
					vout->buffer_size);

			vout->queued_buf_addr[vb->i] = (u8 *) vout->smsshado_phy_addr[vb->i];
			return 0;
		} else {
			ret = rsz_begin(vb->i, vb->i,
					MAX_PIXELS_PER_LINE *\
...
static int omap_vout_open(struct file *file)
{
	...
	do_gettimeofday(&vout->reftime);
printk ("omap_vout_open: reftime: %d|%6d\n", (int)vout->reftime.tv_sec, (int)vout->reftime.tv_usec);
	/* reset rotation back to none */
	vout->rotation = -1;
...
static void check_720(struct omap_vout_device *vout)
{
	if (vout->pix.width == 1280 && machine_has_isp() && !vout->veto_isp_rsz ) {
		if( vout->flg_720 != VIDEO_720_ENABLE ) {
			v4l2_dbg(1, debug, vout->dev->driver, "\nuse isp resizer for 720p!!!\n\n");
			vout->flg_720 = VIDEO_720_ENABLE;
		}
	} else {
		vout->flg_720 = VIDEO_720_DISABLE;
			...
			struct v4l2_format *f)
{
	...
	/* Since we don't know yet if the resizer will be used
	...
	/* Update framebuffer data (a rotation may have been set just
	 * before that call), omap_vout_new_window may not create the
	 * proper window otherwise
	 */
	ovid = &(vout->vid_info);
	ovl = ovid->overlays[0];
			...
			struct v4l2_crop *crop)
{
	...
	if (crop->type == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
		...
		err = omap_vout_new_crop(&vout->pix, &vout->crop, &vout->win,
					 &vout->fbuf, &crop->c,
					 &vout->use_isp_rsz_for_downscale, vout->veto_isp_rsz, vout->force_isp_rsz);
		mutex_unlock(&vout->lock);
		return err;
	} else {
		mutex_unlock(&vout->lock);
			...
			struct v4l2_buffer *buffer)
{
	...
	if (vout->flg_720 == VIDEO_720_ENABLE ||
		...
		if (vout->rsz_configured == 0) {
			...
			isp_rsz_params.out_hsize = (vout->win.w.width + 0x0f) & ~0x0f;
...
void _queue_flush(struct videobuf_queue *q)
{
	...
	*/
	...
	if (vout->linked) {
		...
		for (t = 0; t < ovid->num_overlays; t++) {
			...
			if (ovl->manager && ovl->manager->device) {
				struct omap_overlay_info info;
				struct omap_dss_device *device = ovl->manager->device;
				...
				if (device->caps & OMAP_DSS_DISPLAY_CAP_MANUAL_UPDATE && device->set_update_mode)
					device->set_update_mode(device, OMAP_DSS_UPDATE_AUTO);
			}
		}
	}
...
static int do_streamoff(struct file *file, void *fh, enum v4l2_buf_type i, int keep_display )
{
	struct omap_vout_device *vout = fh;
	int t, r = 0;
	struct omapvideo_info *ovid = &(vout->vid_info);
				...
				for (t = 0; t < ovid->num_overlays; t++) {
					...
					if (ovl->manager && ovl->manager->device) {
						...
						if (ovl->set_overlay_info(ovl, &info))
							return -EINVAL;
						if (device->caps & OMAP_DSS_DISPLAY_CAP_MANUAL_UPDATE && device->set_update_mode)
							device->set_update_mode(device, OMAP_DSS_UPDATE_MANUAL);
					}
				}
			}
...
{
	return do_streamoff(file, fh, i, 0);
}
...
printk ("VIDIOC_DQBUF_FLUSH\n"); 
	...
	case VIDIOC_GET_REFTIME:
	{
		...
		timevalsub(&buffer->timestamp, &vout->reftime );

//printk ("VIDIOC_GET_REFTIME: %6|%6d\n", (int)buffer->timestamp.tv_sec, (int)buffer->timestamp.tv_usec);
		return 0;
	}
	case VIDIOC_STREAMOFF2:
	{
		enum v4l2_buf_type i = *(int *)arg;
printk ("VIDIOC_STREAMOFF2\n"); 
		return do_streamoff(file, fh, i, 1);
	}
	case VIDIOC_VETO_ISP_RSZ:
	{
		int veto = *(int *)arg;
printk ("VIDIOC_VETO_ISP_RSZ: %d\n", veto); 
		vout->veto_isp_rsz = veto;
		return 0;
	}
	case VIDIOC_FORCE_ISP_RSZ:
	{
		int force = *(int *)arg;
printk ("VIDIOC_FORCE_ISP_RSZ: %d\n", force); 
		vout->force_isp_rsz = force;
		return 0;
	}
	default:
printk("omap_vout: unknown ioctl: %08X\n", cmd);
		/* unrecognized ioctl */
		return -ENOIOCTLCMD;
	}
}
...
static int __init omap_vout_setup_video_data(struct omap_vout_device *vout)
{
	...
	vfd->name[strlen(vfd->name) + 1] = '\0';
	vfd->name[strlen(vfd->name)] = vout->vid_info.id + '0';
		...
		int vid_num)
{
	...
	unsigned numbuffers;
	struct video_device *vfd;
	int static_vrfb_allocation = 0, vrfb_num_bufs;

	vout = vid_dev->vouts[vid_num];
	vfd = vout->vfd;

	numbuffers = (vid_num == 0) ? video1_numbuffers : video2_numbuffers;
	vrfb_num_bufs = (vid_num == 0) ? video1_maxbuffers : video2_maxbuffers;
	vout->buffer_size = (vid_num == 0) ? video1_bufsize : video2_bufsize;
	printk(KERN_INFO "Buffer Size = %d, num = %d\n", vout->buffer_size, numbuffers);
	for (i = 0; i < numbuffers; i++) {
...
static int omap_vout_remove(struct platform_device *pdev)
{
	...
	for (k = 0; k < vid_dev->num_displays; k++) {
		if (vid_dev->displays[k]->state != OMAP_DSS_DISPLAY_DISABLED)
			omap_dss_disable_device(vid_dev->displays[k]);
...
static int __init omap_vout_probe(struct platform_device *pdev)
{
	 ...
	 */
	 for (i = 1; i < 3; i++) {
		...
		r = omap_dss_enable_device(def_display);
		if (r) {
...
void omap_vout_isr(void *arg, unsigned int irqstatus)
{
	...
	timevalsub(&timevalue, &vout->reftime );

	if (!vout->first_int && (vout->cur_frm != vout->next_frm)) {
		if( timeval_compare( &timevalue, &vout->cur_frm->ts ) < 0 ) {
//printk(KERN_DEBUG "%8d|%6d  %8d|%6d  wait\r\n", timevalue.tv_sec, timevalue.tv_usec, vout->cur_frm->ts.tv_sec, vout->cur_frm->ts.tv_usec );
			// we are too early for this frame, wait
			spin_unlock(&vout->vbq_lock);
			return;
		}
//printk(KERN_DEBUG "%8d|%6d  %8d|%6d  OUT\r\n", timevalue.tv_sec, timevalue.tv_usec, vout->cur_frm->ts.tv_sec, vout->cur_frm->ts.tv_usec );
	}
	
	if ((cur_display->caps & OMAP_DSS_DISPLAY_CAP_VIDEO_MODE &&
	     cur_display->type == OMAP_DISPLAY_TYPE_DSI &&
	     irqstatus & (DISPC_IRQ_FRAMEDONE | DISPC_IRQ_VSYNC)) ||
	    ((cur_display->type == OMAP_DISPLAY_TYPE_DPI ||
	      cur_display->type == OMAP_DISPLAY_TYPE_HDMI || 
	      cur_display->type == OMAP_DISPLAY_TYPE_DBI) &&
