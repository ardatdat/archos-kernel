...
int omap_vout_new_crop(struct v4l2_pix_format *pix,
	      ...
	      struct v4l2_framebuffer *fbuf, const struct v4l2_rect *new_crop,
		       int *use_isp_rsz_for_downscale, int veto_isp_rsz, int force_isp_rsz)
{
	...
	if (cpu_is_omap24xx() || !machine_has_isp()) {
		...
		if( force_isp_rsz ) {
			printk(KERN_ERR "\n<%s> forcing ISP resizer vresize = %lu\n\n", __func__, vresize);
			*use_isp_rsz_for_downscale = 1;
		} else if (vresize > 4096) {
			if( !veto_isp_rsz ) {
				*use_isp_rsz_for_downscale = 1;
		...
		if (hresize > 4096 ||
			...
			if( !veto_isp_rsz ) {
				*use_isp_rsz_for_downscale = 1;
				...
				if( hresize > 4096 ) {
					printk(KERN_ERR "\n<%s> ISP veto, max 4x H downscale!\n", __func__);
					vresize = 4096;
				}
				if( hresize > 2048 && try_crop.width > 1024) {
					printk(KERN_ERR "\n<%s> ISP veto, max 1024 H width!\n", __func__);
					try_crop.width = 1024;
				}
			}
		}
