		...
		const char *buf, size_t size)
{
	...
	if (enabled) {
 		r = omap_dss_enable_device(dssdev);
		if (r)
			return r;
	} else {
		omap_dss_disable_device(dssdev);
	}
...
static int calc_kb_per_sec(struct omap_dss_device *dssdev)
{
	int kb_per_sec, hz;
...
DSSDBG("%u,%u/%u/%u/%u,%u/%u/%u/%u,%u/%u\n",
			...
			dssdev->panel.config & OMAP_DSS_LCD_IVS ? 1:0);
#endif
		hz = t.pixel_clock * 1000 / ((t.x_res + t.hfp + t.hbp +  t.hsw)*(t.y_res + t.vfp + t.vbp + t.vsw));
		kb_per_sec = t.x_res * t.y_res * hz * 4 / 1000;
//DSSDBG("kb/s: %d, hz: %d\n", kb_per_sec, hz);		
	} else {
  		/*
		 *
  		 * Through-put requirement:
  	 	 * Set OCP freq for 3630 to bandwidth used for active area of display
...
int omap_dss_enable_device(struct omap_dss_device *dssdev)
{
	int r = 0;
	
//DSSDBG("%s, state: %d\n", __FUNCTION__, dssdev->state);	

	if (dssdev->state == OMAP_DSS_DISPLAY_ACTIVE)
		return -1;

	omap_pm_set_min_bus_tput(&dssdev->dev,
  		 	OCP_INITIATOR_AGENT, calc_kb_per_sec(dssdev));
...
EXPORT_SYMBOL(omap_dss_enable_device);

int omap_dss_disable_device(struct omap_dss_device *dssdev)
{
DSSDBG("%s, state: %d\n", __FUNCTION__, dssdev->state);	
	if (dssdev->state != OMAP_DSS_DISPLAY_ACTIVE)
		return 0;
...
EXPORT_SYMBOL(omap_dss_disable_device);

int omap_dss_suspend_device(struct omap_dss_device *dssdev)
{
...
//DSSDBG("%s, state: %d\n", __FUNCTION__, dssdev->state);	
	...
	return dssdev->suspend(dssdev);
}
EXPORT_SYMBOL(omap_dss_suspend_device);
...
static int dss_suspend_device(struct device *dev, void *data)
{
	...
	r = omap_dss_suspend_device(dssdev);
	if (r)
		return r;
...
int omap_dss_resume_device(struct omap_dss_device *dssdev)
{
	int r = 0;	
//DSSDBG("%s, state: %d\n", __FUNCTION__, dssdev->state);	
	if (dssdev->state == OMAP_DSS_DISPLAY_ACTIVE)
...
EXPORT_SYMBOL(omap_dss_resume_device);
...
static int dss_resume_device(struct device *dev, void *data)
{
	...
	r = omap_dss_resume_device(dssdev);
	if (r) {
		return r;
	}
