...
void dispc_set_dither_mode(enum omap_dither_mode mode)
{
	int type;
	int enable;
...
static void dispc_error_worker(struct work_struct *work)
{
	...
	if (errors & DISPC_IRQ_SYNC_LOST_DIGIT) {
		...
		for (i = 0; i < omap_dss_get_num_overlay_managers(); ++i) {
			...
			if (mgr->device == NULL) {
				DSSERR("TV: manager enabled without device?\n");
				break;
			}
		...
		if (manager) {
			for (i = 0; i < omap_dss_get_num_overlays(); ++i) {
				...
				ovl = omap_dss_get_overlay(i);
				if (ovl == NULL) {
					DSSERR("disabling a NULL overlay?\n");
					break;
				}
...
int dispc_wait_for_vsync(enum omap_channel channel) 
{
	dispc_go(channel);
	return omap_dispc_wait_for_irq_timeout(DISPC_IRQ_EVSYNC_EVEN | DISPC_IRQ_EVSYNC_ODD, msecs_to_jiffies(100));
}
