		...
		struct omap_dss_device *dssdev)
{
	...
	for (i = 0; i < mgr->num_overlays; i++) {
		...
		if (r) {
			DSSERR("display '%s' on manager '%s' failed on check_overlay\n", dssdev->name, mgr->name);
			return r;
		}
	}
...
/* Configure dss for partial update. Return possibly modified update
 * area */
		...
		u16 *xi, u16 *yi, u16 *wi, u16 *hi)
{
	...
	DSSDBG("dss_setup_partial_planes %d,%d %dx%d\n",
		*xi, *yi, *wi, *hi);
...
int dss_init_overlay_managers(struct platform_device *pdev)
{
	...
	for (i = 0; i < 2; ++i) {
		...
		case 0:
			...
			mgr->supported_displays =
				...
				OMAP_DISPLAY_TYPE_HDMI | OMAP_DISPLAY_TYPE_DUMMY ;
			break;
		case 1:
			mgr->name = "tv";
...
#ifdef L4_EXAMPLE
	{
		...
		mgr->name = "l4";
		mgr->supported_displays =
			OMAP_DISPLAY_TYPE_DBI | OMAP_DISPLAY_TYPE_DSI  | OMAP_DISPLAY_TYPE_DUMMY | OMAP_DISPLAY_TYPE_HDMI;
