...
void omap_rfbi_write_command(const void *buf, u32 len)
{
	...
	case OMAP_DSS_RFBI_PARALLELMODE_16:
	{
		...
		BUG_ON(len & 1);
		for (; len; len -= 2) {
			 while(rfbi_read_reg(RFBI_SYSSTATUS) & BUSY) {;}
		...
		int pix_dfm)
{
	...
	if (rfbi.datatype == OMAP_DSS_RFBI_DATATYPE_18 &&
	   rfbi.parallelmode == OMAP_DSS_RFBI_PARALLELMODE_16) {
		const u32 __iomem *pd = (u32 *)buf;
		pd += start_offset;

		if ( pix_dfm == OMAP_DSS_RFBI_DFM_RGB18_2 ) {
			// 18 bits in 6 + 12
			for (; h; --h) {
				for (i = 0; i < w; ++i) {
					...
					val = ((__raw_readb(b) & 0xfc));
					b++;
					val |= ((__raw_readb(b) & 0xfc) << 8);
					b++;
					val |= ((__raw_readb(b) & 0xfc) << 16);

					rfbi_write_reg(RFBI_PARAM, (u16) (val >> 16));
					rfbi_write_reg(RFBI_PARAM, (u16) (val));
			...
			for (; h; --h) {
				for (i = 0; i < w; ++i) {
					...
					u32 val;
					const u8 __iomem *b = (const u8 __iomem *)pd;
					val = ((__raw_readb(b) & 0xfc) >> 2 );
					b++;
					val |= ((__raw_readb(b) & 0xfc) >> 2 ) << 6;
					b++;
					val |= ((__raw_readb(b) & 0xfc) >> 2 ) << 12;

					rfbi_write_reg(RFBI_PARAM, (u16) (val >> 2));
					rfbi_write_reg(RFBI_PARAM, (u16) (val & 0x03));
					++pd;
				}
				pd += horiz_offset;
			}
...
int rfbi_configure(int rfbi_module, int bpp, int lines)
{
	...
	case OMAP_DSS_RFBI_CYCLEFORMAT_2_1:
		cycle1 = lines;
		cycle2 = (bpp-lines) | (2*lines-bpp) << 8;
		break;
...
static int do_update(struct omap_dss_device *dssdev, struct update_region *upd)
{
	...
	if (dssdev->manager->caps & OMAP_DSS_OVL_MGR_CAP_DISPC) {
		...
		omap_rfbi_write_pixels(addr, scr_width, x, y, w, h, dssdev->phy.rfbi.pixel_dfm );
...
{
	struct omap_dss_device *dssdev = rfbi.dssdev[rfbi_module];
	struct update_param p;
	...
	/* set the CPU latency to minimum and keep it there for 50ms */
	omap_pm_set_max_mpu_wakeup_lat(&dssdev->dev, 10);
	cancel_delayed_work(&rfbi.update[rfbi_module].latency_work);
	schedule_delayed_work(&rfbi.update[rfbi_module].latency_work, 
			msecs_to_jiffies(50));
			...
			u16 x, u16 y, u16 w, u16 h)
{
	...
	if (rfbi.update[rfbi_module].mode != OMAP_DSS_UPDATE_MANUAL)
		return 0;
	...
	struct omap_dss_device *dssdev)
{
	...
	rfbi_module = rfbi_find_display(dssdev);
	return rfbi.update[rfbi_module].mode;
}

static void update_work_func(struct work_struct *work)
{
	struct omap_dss_device *dssdev;
	struct update_struct *update;
...
static void latency_work_func(struct work_struct *work)
{
	struct omap_dss_device *dssdev;
	struct update_struct *update;
	
	update = container_of(work, struct update_struct, latency_work);
	dssdev = rfbi.dssdev[update->rfbi_module];
	omap_pm_set_max_mpu_wakeup_lat(&dssdev->dev, -1);
}
...
static int rfbi_display_enable(struct omap_dss_device *dssdev)
{
	...
	r = omap_dispc_register_isr(framedone_callback, dssdev,
			DISPC_IRQ_FRAMEDONE);
	if (r) {
	...
	return 0;
err2:
	omap_dispc_unregister_isr(framedone_callback, dssdev,
			DISPC_IRQ_FRAMEDONE);
err1:
...
int rfbi_init_display(struct omap_dss_device *dssdev)
{
	...
	rfbi_module = dssdev->phy.rfbi.channel;
	rfbi.dssdev[rfbi_module] = dssdev;

	dssdev->caps = OMAP_DSS_DISPLAY_CAP_MANUAL_UPDATE;

	rfbi.update_delay = msecs_to_jiffies(20);
	rfbi.update[rfbi_module].mode = OMAP_DSS_UPDATE_MANUAL;
	rfbi.update[rfbi_module].rfbi_module = rfbi_module;
	INIT_DELAYED_WORK(&rfbi.update[rfbi_module].polling_work, update_work_func);
	INIT_DELAYED_WORK(&rfbi.update[rfbi_module].latency_work, latency_work_func);
