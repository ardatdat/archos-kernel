...
static int omapfb_crop_plane(struct fb_info *fbi, struct omapfb_plane_info *pi)
{
	struct omapfb_info *ofbi = FB2OFB(fbi);
	struct omapfb2_device *fbdev = ofbi->fbdev;
	struct omap_dss_device *display = fb2display(fbi);
	...
	info.pos_x = pi->pos_x;
	info.pos_y = pi->pos_y;
	info.width = info.out_width = pi->out_width;
	info.height = info.out_height = pi->out_height;
	info.enabled = pi->enabled;

	if (pi->enabled) {
		struct fb_var_screeninfo *var = &fbi->var;
		// special crop call, we make the var info match the plane info
		var->xres	= pi->out_width;
		var->yres	= pi->out_height;
		var->xoffset = pi->pos_x;
		var->yoffset = pi->pos_y;
	}
...
out:
	if (r)
		dev_err(fbdev->dev, "crop_plane failed\n");
	return r;
}
