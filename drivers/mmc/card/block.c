...
static void mmc_blk_put(struct mmc_blk_data *md)
{
	...
	if (md->usage == 0) {
		int i;
		for (i=0; i<md->num_disks; i++) {
			int devidx = MINOR(disk_devt(md->disk[i])) >> MMC_SHIFT;
...
mmc_blk_set_blksize(struct mmc_blk_data *md, struct mmc_card *card)
{
	...
	if (err) {
		printk(KERN_ERR "%s: unable to set block size to %d: %d\n",
			md->disk[0]->disk_name, cmd.arg, err);
		return -EINVAL;
	}
...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	do {
		...
		brq.cmd.arg = req->sector + md->disk_start[disk_index];
		if (!mmc_card_blockaddr(card))
			brq.cmd.arg <<= 9;
...
//printk("ret: %d, hostid: %d, cardidx: %d\n", ret, host_id, cardidx);	
	...
	for (i=0; i<md->num_disks; i++) {	
		devidx[i] = find_first_zero_bit(dev_use, MMC_NUM_MINORS);
		if (devidx[i] >= MMC_NUM_MINORS) {
			ret = -ENOSPC;
			goto err_kfree;
		}
		__set_bit(devidx[i], dev_use);

		md->disk[i] = alloc_disk(1 << MMC_SHIFT);
		if (md->disk[i] == NULL) {
			ret = -ENOMEM;
			goto err_putdisk;
		}
	...
	for (i=0; i<initialized; i++) {	
		...
		md->disk[i]->first_minor = devidx[i] << MMC_SHIFT;
		...
		sprintf(md->disk[i]->disk_name, "mmcblk%d", devidx[i]);
