...
static int max_dma_ch = 8;
module_param_named(max_dma, max_dma_ch, int, S_IRUGO|S_IWUSR);
...
static irqreturn_t mmc_omap_irq(int irq, void *dev_id)
{
	...
	/* PIO mode */
	if (data && host->buffer && host->bytesleft) {

		/* ToDo: handle more than one entry in scatterlist */

		if (status & BRR) {
			int k;
			for (k = 0; k < data->blksz / 4; k++) {
				*(host->buffer)++ = OMAP_HSMMC_READ(host->base, DATA);
			}
			
			host->bytesleft -= data->blksz;		
		}else if (status & BWR) {
			int k;
			for (k = 0; k < data->blksz / 4; k++) {
...
/*
 * Add scatterlist segments to DMA chain
 */
static int
mmc_omap_load_dma_chain(int sync_dir, struct mmc_omap_host *host, 
		struct mmc_data *data)
{
	...
	next_sg = &data->sg[host->sg_done];
	num_segs = min((int)data->sg_len - host->sg_done, host->max_dma_ch);
	
	for_each_sg(next_sg, sg, num_segs, chained) {
		int frames = sg_dma_len(sg) / data->blksz;
		
		if (sync_dir)			
			err = omap_dma_chain_a_transfer(host->dma_ch,
					(dma_addr_t)(host->mapbase + OMAP_HSMMC_DATA),
					sg_dma_address(sg), (data->blksz / 4),
					frames, host);
		else
			err = omap_dma_chain_a_transfer(host->dma_ch,
					sg_dma_address(sg),
					(dma_addr_t)(host->mapbase + OMAP_HSMMC_DATA),
					(data->blksz / 4), frames, host);
		...
		if (err != 0) {
			dev_err(mmc_dev(host->mmc),
			  "%s: omap_dma_chain_a_transfer() failed during %s with %d\n",
			  mmc_hostname(host->mmc), sync_dir ? "read":"write", err);
			return err;
		}
		
		/*dev_dbg(mmc_dev(host->mmc), 
			"sync_dir:%i sg[%i] addr:0x%08x dma_len:%u frames:%i\n",
			sync_dir, chained+host->sg_done, sg_dma_address(sg), sg_dma_len(sg), frames);*/
		
	}

	host->sg_done += chained;
	return chained;
}
 ...
 */
static void mmc_omap_dma_cb(int lch, u16 ch_status, void *dev_id)
{
	struct mmc_omap_host *host = dev_id;
	...
	/* DMA not complete yet */
	if (omap_dma_chain_status(host->dma_ch) == OMAP_DMA_CHAIN_ACTIVE)
		return;
	...
	if (host->sg_done < data->sg_len) {			
		mmc_omap_load_dma_chain(!(data->flags & MMC_DATA_WRITE), 
				host, data);
		return;
	}
...
mmc_omap_start_dma_transfer(struct mmc_omap_host *host, struct mmc_request *req)
{
	...
	/* check this constraint early, simplify code further down */
	if ((data->blksz % 4) != 0)
	...
	if (host->dma_ch != -1) {
		dev_info(mmc_dev(host->mmc), 
				"dma still active on next request?\n");
	...
	if (!(data->flags & MMC_DATA_WRITE)) {
		...
		params.src_or_dst_synch = OMAP_DMA_SRC_SYNC;
		params.trigger = sync_dev;
	} else {
		sync_dir = 0;
		...
		params.src_or_dst_synch = OMAP_DMA_DST_SYNC;
		params.trigger = sync_dev;
	}

	ret = omap_request_dma_chain(sync_dev, "MMC/SD", 
			mmc_omap_dma_cb, &dma_ch, 
			min(host->max_dma_ch, (int)data->sg_len), 
	...
	mmc_omap_load_dma_chain(sync_dir, host, data);		
	omap_start_dma_chain_transfers(dma_ch);
	return 0;
}
...
mmc_omap_prepare_data(struct mmc_omap_host *host, struct mmc_request *req)
{
	...
	if (host->use_dma) {
		...
		if ((req->data->blksz % 4) != 0) {
			dev_err(mmc_dev(host->mmc), "blksz is non multiple of four\n");
			return -1;
		}
		
		host->bytesleft = req->data->blocks * req->data->blksz;
		host->buffer = sg_virt(req->data->sg);
...
//printk("%d -> OMAP_MMC_MASTER_CLOCK(%d)/%d == %d\n", ios->clock, OMAP_MMC_MASTER_CLOCK, dsor, OMAP_MMC_MASTER_CLOCK/dsor);
