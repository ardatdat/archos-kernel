...
static inline int gpio_valid(int gpio)
{
	...
	if ((cpu_is_omap16xx()) && gpio < 64)
		return 0;
	if ((cpu_is_omap730() || cpu_is_omap34xx()) && gpio < 192)
		return 0;
...
#endif
#ifdef CONFIG_ARCH_OMAP34XX
void omap3_gpio_prepare_per_off(int core_off)
{
	...
	while (pad->gpio >= 0) {
		...
		/* stupid hack to not treat GPIO 170 & 57*/
		if ( (pad->gpio == 170 ) || (pad->gpio == 57) || (pad->gpio == 124))
			goto next_pad;
		...
		if (!(tmp_oe & pin)) {
			...
			/* if the current pad has off mode override -> skip */
			if (core_off && (pad->save & OMAP3_OFF_EN))
				goto next_pad;
			...
			/* make sure that the pad configuration is established */
			omap_ctrl_readw(offset);
