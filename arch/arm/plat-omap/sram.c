...
#define CM_DSS_ICLK_ST  	       (1 << 0)
#define DISPC_VSYNC_ST  	       (1 << 1)

#ifdef CONFIG_OMAP2_DSS
static u32 dispc_base = 0;
#endif
...
#ifdef CONFIG_OMAP2_DSS
static inline void dispc_write_reg(u16 idx, u32 val)
{
	if (!dispc_base)
		return;
...
static inline void wait_for_vsync(void)
{
	...
	v = __raw_readl(OMAP34XX_CM_REGADDR(OMAP3430_DSS_MOD, CM_ICLKEN));
	if (!(v & CM_DSS_ICLK_ST))
		return;

	v = dispc_read_reg(DISPC_CONTROL);
	if (!(v & DISPC_LCD_EN))
		return;

	v = dispc_read_reg(DISPC_IRQENABLE);
	if (!(v & DISPC_VSYNC_ST))
		dispc_write_reg(DISPC_IRQSTATUS, DISPC_VSYNC_ST);

	while(1) {
		v = dispc_read_reg(DISPC_IRQSTATUS);
		if (v & DISPC_VSYNC_ST)
			break;

		//printk("dispc_irqstatus: %08x\n", v);
	}
}
#endif
...
#ifdef CONFIG_OMAP2_DSS
	dispc_base = (u32)ioremap(DISPC_BASE, DISPC_SZ_REGS);
	if (!dispc_base) {
		printk("%s: can't ioremap DISPC\n", __FUNCTION__);
	}
#endif
