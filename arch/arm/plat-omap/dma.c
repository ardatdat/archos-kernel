...
static spinlock_t dma_reg_lock;
...
static inline void omap2_enable_irq_lch(int lch)
{
	...
	spin_lock_irqsave(&dma_reg_lock, flags);
	val = dma_read(IRQENABLE_L0);
	val |= 1 << lch;
	dma_write(val, IRQENABLE_L0);
	spin_unlock_irqrestore(&dma_reg_lock, flags);
}

static inline void omap2_dma_disable_irq_lch(int lch)
{
	u32 val;
	unsigned long flags;
...
void omap_disable_lch_and_drain_fifo(int lch)
{
	u32 l;
	u32 syscfg_save, v;
	 ...
	 * Put the sDMA in No-Idle mode first, then disable the DMA
	 * channel and wait for the draining to complete, and finally
	 * put the sDMA module back in its previous standby mode.
	 */
	if (cpu_is_omap34xx()) {
		syscfg_save = dma_read(OCP_SYSCONFIG);
		v = syscfg_save;
		v &= ~DMA_SYSCONFIG_MIDLEMODE_MASK;
		v |= DMA_SYSCONFIG_MIDLEMODE(DMA_IDLEMODE_NO_IDLE);
	...
	do {
		...
		l &= ~OMAP_DMA_CCR_EN;
		dma_write(l, CCR(lch));
	} while (omap_get_dma_rd_wr_active_status(lch));

	if (cpu_is_omap34xx())
		dma_write(syscfg_save , OCP_SYSCONFIG);
}
EXPORT_SYMBOL(omap_disable_lch_and_drain_fifo);
...
static irqreturn_t omap2_dma_irq_handler(int irq, void *dev_id)
{
	...
	spin_lock(&dma_reg_lock);
	enable_reg = dma_read(IRQENABLE_L0);
	spin_unlock(&dma_reg_lock);
