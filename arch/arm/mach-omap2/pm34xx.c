...
void omap_sram_idle(void)
{
	...
	if (per_next_state < PWRDM_POWER_ON) {
		...
		if (per_next_state == PWRDM_POWER_OFF) {
			...
			/* XXX WA for OMAP3430 errata i468, applies to all
			 * OMAP3 apparently.*/
			omap3_gpio_prepare_per_off(
					core_next_state == PWRDM_POWER_OFF);

			/* XXX on OMAP3630, GPIO pad isolation remove glitch
			 * can be circumvented by removing the isolation
			 * under software control
			 */
			if (core_next_state == PWRDM_POWER_OFF &&
					omap_rev() >= OMAP3630_REV_ES1_0) {
				prm_set_mod_reg_bits(OMAP3630_PAD_OFF_MODE_OVR,
	...
	*/
	...
	if (per_next_state == PWRDM_POWER_OFF) {
		...
		/* 
		 * XXX we only need to restore the pad context if
		 * full system OFF mode was attempted
		 */
		if (core_next_state == PWRDM_POWER_OFF) {
			if (omap_rev() >= OMAP3630_REV_ES1_0)
				prm_clear_mod_reg_bits(OMAP3630_PAD_OFF_MODE_OVR,
					OMAP3430_GR_MOD, OMAP3_PRM_VOLTCTRL_OFFSET);
		}
	}
...
int set_pwrdm_state(struct powerdomain *pwrdm, u32 state)
{
	 ...
	 * REVISIT: OMAP3611 has no DSP but power domain must still be switched OFF
	 *           to achieve SYS_OFF_MODE
	 */
	if ((omap_ctrl_readl(OMAP36XX_CONTROL_FEATURE_OMAP_STATUS) & BIT(12)) == 0) {
...
static int omap3_pm_suspend(void)
{
	...
	if (!pwrdm_can_idle(core_pwrdm)) {
		printk(KERN_DEBUG "omap3_pm_suspend: CORE cannot idle!\n");
		printk(KERN_DEBUG "\tCM_FCLKEN1: %08x CM_FCLKEN3: %08x\n", 
			cm_read_mod_reg(core_pwrdm->prcm_offs, CM_FCLKEN1), 
			cm_read_mod_reg(core_pwrdm->prcm_offs, OMAP3430ES2_CM_FCLKEN3));
		ret = -EAGAIN;
		goto restore;
	}
