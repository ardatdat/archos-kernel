...
EXPORT_SYMBOL(usb_musb_disable_autoidle);
...
static struct musb_hdrc_platform_data musb_plat = {
	...
	/* Charge pump on TWL4030 can supply up to
	 * 100 mA ... This is the default value and can be overwritten
	 * by board data
	 */
...
void __init usb_musb_init(void)
{
	...
	if ( usb_cfg ) {
		...
		if ( GPIO_EXISTS( gio_usb_pwroff ) ) {
			...
			GPIO_INIT_OUTPUT( gio_usb_pwroff );
			musb_plat.set_vbus = set_vbus;
			
			set_vbus( &musb_device.dev, 0 );
			
			if ( usb_cfg->rev[hardware_rev].usb_max_power )
				musb_plat.power = usb_cfg->rev[hardware_rev].usb_max_power/2;
		}
	}
