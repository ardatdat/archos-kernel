...
{
	pr_debug("[archos-lcd-a35] panel_init [%s]\n", ddata->name);
...
{
	pr_info("[archos-lcd-a35] panel_enable [%s]\n", disp->name);
	if (panel_state == 1)
		return -1;
...
{
	pr_debug("[archos-lcd-a35] panel_disable [%s]\n", disp->name);
...
static struct omap_dss_device mt_a35_panel = {
	.type = OMAP_DISPLAY_TYPE_DBI,
	.name = "lcd",
	.driver_name = "wt_lcd_35",
	...
	.panel = {
		...
		.timings = {
			...
			.y_res		= 480,
			// 60 hz
			.pixel_clock	= 27000, 	/* 27MHz */
			.hsw		= 18,		/* horizontal sync pulse width */
			.hfp		= 80,		/* horizontal front porch */
			.hbp		= 20,		/* horizontal back porch */
			.vsw		= 2,		/* vertical sync pulse width */
			.vfp		= 4,		/* vertical front porch (forced to 0, hardware restriction */
...
int __init panel_a35_init(struct omap_dss_device *disp_data)
{
	...
	panel_init(&mt_a35_panel);

	*disp_data = mt_a35_panel;
