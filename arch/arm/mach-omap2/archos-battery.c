...
static int get_on_dcin(struct archos_battery *bat)
{
	if ( bat->gpio_dc_detect ) {
		return gpio_get_value_cansleep(bat->gpio_dc_detect);
	} else
		return bat->on_dcin;
}
...
int archos_get_dcin_plug(void)
{
	if ( (bat != NULL) && (bat->gpio_dc_detect) )
		return ( bat->old_dcin ? 0 : gpio_get_value_cansleep(bat->gpio_dc_detect) );
...
static ssize_t battery_show_voltage_max_design(struct device* dev, 
		    struct device_attribute *attr, char* buf)
{
	struct archos_battery *bat = dev_get_drvdata(dev);
	
	return snprintf(buf, PAGE_SIZE, "%i\n", bat->voltage_max_design);
}
static ssize_t battery_store_voltage_max_design(struct device* dev,
		    struct device_attribute *attr, const char* buf, size_t len)
{
	struct archos_battery *bat = dev_get_drvdata(dev);
	...
	if (new_mV != bat->voltage_max_design) {
		bat->voltage_max_design = new_mV;
	}
	return len;	
}
static DEVICE_ATTR(voltage_max_design, S_IRUGO|S_IWUSR,
    battery_show_voltage_max_design, battery_store_voltage_max_design);

static ssize_t battery_show_voltage_min_design(struct device* dev, 
		    struct device_attribute *attr, char* buf)
{
	struct archos_battery *bat = dev_get_drvdata(dev);
...
static ssize_t battery_store_voltage_min_design(struct device* dev,
		    struct device_attribute *attr, const char* buf, size_t len)
{
	struct archos_battery *bat = dev_get_drvdata(dev);
	...
	if (new_mV != bat->voltage_min_design) {
		bat->voltage_min_design = new_mV;
	}
	return len;	
}

static DEVICE_ATTR(voltage_min_design, S_IRUGO|S_IWUSR,
    battery_show_voltage_min_design, battery_store_voltage_min_design);
