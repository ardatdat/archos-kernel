...
static int __init archos_lcd_panel_init(struct omap_dss_device *disp_data)
{
	...
	switch (hardware_rev) {
	default:
		return panel_boe_wsvga_10_init(disp_data);
	}
}
...
#ifdef CONFIG_PM
struct vout_platform_data a10_vout_data = {
		...
		unsigned gpio, unsigned ngpio)
{
	...
	if ( hardware_rev > 1 ) {
		...
		board_usb_data.gpio_usb_id = gpio + 1;
	
		gpio_request(board_usb_data.gpio_usb_id, "USB_ID");
		gpio_direction_input(board_usb_data.gpio_usb_id);
		/* check - do we need to change pullup/pulldown configuration? */
		board_usb_data.usb_id_irq = gpio_to_irq(board_usb_data.gpio_usb_id);
	}
	return 0;
}
...
static void archos_hdmi_power(struct i2c_client *cl, int on_off)
{
	...
	if (on_off)
		gpio_direction_output(pdata->pwr_gpio, 0);
	else
...
{
	/* driver will manage this GPIO, just apply the pin multiplexing */
	...
	gpio_set_value(GPIO_PIN(disp_conf->hdmi_pwr), 0);
...
static void enable_5V_supply_init(void)
{

	if (supply_5V_config.nrev <= hardware_rev)
		return;

	omap_cfg_reg(GPIO_MUX(supply_5V_config.rev[hardware_rev].pwr_on));
	archos_gpio_init_output(&supply_5V_config.rev[hardware_rev].pwr_on, "5v pwr");
	/* Enable board 5V at start */
	gpio_set_value(GPIO_PIN(supply_5V_config.rev[hardware_rev].pwr_on), 1);
}
...
static void init_unused(void)
{
	...
	gpio_request(26, "gps_shtdwn");
	omap_cfg_reg( AG7_3430_GPIO26 );
	gpio_direction_input(26); // we have a pull-down
