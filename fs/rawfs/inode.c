...
int rawfs_file_ioctl(struct inode *inode, struct file *filp,
...
static int rawfs_write_inode(struct inode *inode, int wait)
{
	struct super_block *sb = inode->i_sb;
	struct buffer_head *bh;
	struct rawfs_section_header *section_header;
...
static int rawfs_get_sb(struct file_system_type *fs_type,
	int flags, const char *dev_name, void *data, struct vfsmount *mnt)
{
	return get_sb_bdev(fs_type, flags, dev_name, data, rawfs_fill_super,
			   mnt);
}
