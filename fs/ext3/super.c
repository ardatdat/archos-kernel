...
#ifdef CONFIG_EXT3_FS_UMODE
	if (sbi->s_fmode != EXT3_DEF_UMODE ) {
		seq_printf(seq, ",fmode=%04o", sbi->s_fmode);
	}
	if (sbi->s_dmode != EXT3_DEF_UMODE ) {
...
enum {
	...
	Opt_grpquota, Opt_umode, Opt_fmode, Opt_dmode,
};
...
set_qf_format:
		...
		case Opt_umode:
			if (match_octal(&args[0], &option))
				return 0;
			sbi->s_fmode = sbi->s_dmode = option;
			break;
		case Opt_fmode:
			if (match_octal(&args[0], &option))
				return 0;
...
#ifdef CONFIG_EXT3_FS_UMODE
	sbi->s_fmode  = EXT3_DEF_UMODE;
	sbi->s_dmode  = EXT3_DEF_UMODE;
#endif
