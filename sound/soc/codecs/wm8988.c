...
/*
 * write to the WM8988 register space
 */
...
void wm8988_power_up_inputs(struct snd_soc_codec *codec, int input_blocs, int on)
{
#if !defined (DYNAMIC_POWER)
	u16 pwr1_reg = snd_soc_read(codec, WM8988_PWR1);
	...
	snd_soc_write(codec, WM8988_PWR1, pwr1_reg);
#endif /* DYNAMIC_POWER */
}
EXPORT_SYMBOL_GPL(wm8988_power_up_inputs);

void wm8988_select_outputs(struct snd_soc_codec *codec, int outputs, int on)
{
#if !defined (DYNAMIC_POWER)
	u16 pwr2_reg = snd_soc_read(codec, WM8988_PWR2);
	...
	snd_soc_write(codec, WM8988_PWR2, pwr2_reg);
#endif /* DYNAMIC_POWER */
}
EXPORT_SYMBOL_GPL(wm8988_select_outputs);

/*
 * WM8988 Controls
 */
static int wm8988_put_volsw_2r_vu(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	val = wm8988_read_reg_cache(codec, reg);
	wm8988_write(codec, reg, val | OUTVU);
	return 1;
}

/* Allows to set the update bit when a vol is updated */
#define SOC_WM8988_DOUBLE_R(xname, reg_left, reg_right, xshift, xmax, xinvert) \
{	.iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = (xname), \
	.info = snd_soc_info_volsw_2r, \
	.get = snd_soc_get_volsw_2r, .put = wm8988_put_volsw_2r_vu, \
...
SOC_DOUBLE_R_TLV("Capture Digital Volume", WM8988_LADC, WM8988_RADC,
		 0, 255, 0, adc_tlv),
SOC_WM8988_DOUBLE_R("Capture Volume", WM8988_LINVOL, WM8988_RINVOL,
		 0, 63, 0),
...
SOC_DOUBLE_R("Headphone Playback ZC Switch", WM8988_LOUT1V,
	     WM8988_ROUT1V, 7, 1, 0),
SOC_WM8988_DOUBLE_R("Headphone Playback Volume", WM8988_LOUT1V, WM8988_ROUT1V,
		 0, 127, 0),

SOC_DOUBLE_R("Speaker Playback ZC Switch", WM8988_LOUT2V,
	     WM8988_ROUT2V, 7, 1, 0),
SOC_WM8988_DOUBLE_R("Speaker Playback Volume", WM8988_LOUT2V, WM8988_ROUT2V,
		 0, 127, 0),


SOC_DOUBLE_R("PGA Capture Boost Capture Volume", WM8988_LADCIN, WM8988_RADCIN, 4, 3, 0),

#if !defined (DYNAMIC_POWER) || defined (COMPATIBILITY)
// Compatibility controls have to be defined here
#endif
...
static const struct snd_kcontrol_new wm8988_right_line_controls =
	SOC_DAPM_VALUE_ENUM("Route", wm8988_rline_enum);
				...
				struct snd_soc_dai *dai)
{
	...
	case 1:
		/* Left DAC to Right OUT */
		snd_soc_write(codec, WM8988_ROUTM1, 0x100);
		snd_soc_write(codec, WM8988_ROUTM2, 0x00);
		break;
	case 2:
		/* Swap right and left channels (32 bits DMA needs this) */
		iface |= 0x20;
		/* Right DAC to Right OUT */
		snd_soc_write(codec, WM8988_ROUTM1, 0x00);
		snd_soc_write(codec, WM8988_ROUTM2, 0x100);
		break;
	}	
				 ...
				 enum snd_soc_bias_level level)
{
	...
	u16 pwr2_reg = snd_soc_read(codec, WM8988_PWR2) & ~0x180;
#endif
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->bias_level == SND_SOC_BIAS_OFF) {
			...
			msleep(100);
#ifndef DYNAMIC_POWER
			/* VREF, VMID=2x50k, ADC enabled, digital enabled */
			snd_soc_write(codec, WM8988_PWR1, pwr_reg | 0x00cc);
			/* DAC enabled */
			snd_soc_write(codec, WM8988_PWR2, pwr2_reg | 0x0180);
#endif
		}
...
static const int wm8988_restore_seq[] = {
	5, 23, 24, 7, 8, 12, 13, 16,  27, 31, 32, 33,
...
#if 1
	for (i = 0; i < ARRAY_SIZE(wm8988_restore_seq); i++) {
		int r = wm8988_restore_seq[i];
		
		data[0] = (r << 1) | ((cache[r] >> 8) & 0x0001);
		data[1] = cache[r] & 0x00ff;
		i2c_smbus_write_byte_data(codec->control_data, data[0], data[1]);		
	}
#else
...
static void wm8988_init_default(struct snd_soc_codec *codec)
{
	/* 3D */
	/* put 3D on playback */
	snd_soc_write(codec, WM8988_3D, 0x0080);

	/* Volumes */
	/* ADC and DAC to 0 dB, speaker and
	   headphone to 1/2*max, input to 0 */
	snd_soc_write(codec, WM8988_LADC, 0x1C3);
	snd_soc_write(codec, WM8988_RADC, 0x1C3);
	snd_soc_write(codec, WM8988_LDAC, 0x01FF);
	snd_soc_write(codec, WM8988_RDAC, 0x01FF);
	snd_soc_write(codec, WM8988_LOUT1V, 0x013F);
	snd_soc_write(codec, WM8988_ROUT1V, 0x013F);
	snd_soc_write(codec, WM8988_LOUT2V, 0x013F);
	snd_soc_write(codec, WM8988_ROUT2V, 0x013F);
	snd_soc_write(codec, WM8988_LINVOL, 0x0100);
	snd_soc_write(codec, WM8988_RINVOL, 0x0100);

	/* Routes */
	/* PGA connected to ADC in stereo,
	   INPUT1 connected to PGA, no boost */
	snd_soc_write(codec, WM8988_ADCIN, 0x00);
	snd_soc_write(codec, WM8988_LADCIN, 0x00);
	snd_soc_write(codec, WM8988_RADCIN, 0x00);
	/* Left DAC to left mixer, right DAC to right
	   mixer, INPUT1 selected as input to mix
	   but not connected to mixers, everything
	   else disabled */
	snd_soc_write(codec, WM8988_LOUTM1, 0x100);
	snd_soc_write(codec, WM8988_LOUTM2, 0x00);
	snd_soc_write(codec, WM8988_ROUTM1, 0x00);
	snd_soc_write(codec, WM8988_ROUTM2, 0x100);

	/* Configurations */
	/* Low power mode */
	snd_soc_write(codec, WM8988_LPPB, 0x08);
	/* Best SNR, no choice anyway since we are in
	   USB mode (master clock = 12MHz), LRC disabled
	   only if DAC and ADC are disabled, LCOM and
	   HPCOM enabled */
	snd_soc_write(codec, WM8988_ADCTL2, 0x184);
	/* Analogue bias optimized for 3.3V (lowest
	   power consumption for any voltage), Thermal
	   shutdown enabled */
	snd_soc_write(codec, WM8988_ADCTL1, 0x1C0);
}
...
static int wm8988_register(struct wm8988_priv *wm8988)
{
	...
	printk(KERN_INFO "WM8988 initialized\n");
	return 0;
